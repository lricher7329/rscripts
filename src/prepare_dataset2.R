...
df <- keto_dataset %>%
  rename(record_id=id) %>%
  mutate(consent=included) %>%
  mutate(randomized=included) %>%
  mutate(tx_arm=if_else(arm==1,"keto/metoclo","plac/metoclo")) %>%
  mutate(treat=NA) %>%
  mutate(treated=if_else(is.na(lfu_reason) & is.na(tx_arm),NA,"Yes")) %>%
  rename(lfu1=lfu_reason) %>%
  mutate(lfu2=if_else(treated=="Yes" & is.na(fupsatis),"No response",NA))


df <- df %>%
  select(sid,included,excluded_reason,consent,randomized,record_id,arm,tx_arm,treated,lfu1,lfu2,site:fupmeds)



write_csv(df,"data/ed_migraine_data.csv")
